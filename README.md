...
部署并绑定名为 `KV` 的命名空间后，管理员登录 `/admin/tokens` 即可打开 Token 管理页面。

- 默认超时时间单位为分钟，默认值为 `43200`（30 天），只影响之后创建的 Token。
- 创建 Token 时可以单独指定超时时间；设置为 `0` 会立即禁用该 Token。
- 已有 Token 可以修改为新的分钟数，或直接点击禁用；修改正数会从修改时刻重新计算到期时间。
- Token 到期或被禁用后，`/sub?token=...` 返回 `410`，订阅转换使用的临时 Token 也会同步失效。
- 每个新创建的 Token 会绑定独立 UUID；Token 到期或禁用后，使用该 Token 生成的节点无法建立新的 WebSocket、gRPC 或 XHTTP 连接。
- 已经建立的长连接不会被主动定时断开；客户端重连后会重新进行有效期校验。旧版默认 Token 节点继续绑定默认 UUID，并随默认 Token 状态控制。

Token 数据保存在 KV 的 `subtokens.json`，默认设置保存在 `subtoken_settings.json`。
